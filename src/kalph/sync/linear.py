...
Branch naming: issues drive `kalph/<issue-identifier>-<slug>` branches so
...
_ISSUES_QUERY = """
query KalphIssues($filter: IssueFilter) {
...
_COMMENT_MUTATION = """
mutation KalphComment($issueId: String!, $body: String!) {
...
class LinearAdapter:
    ...
    def fetch_issues(self) -> list[InboundIssue]:
        ...
        for node in data.get("issues", {}).get("nodes", []):
            ...
            # (70-89) so tracker issues outrank kalph-proposed work but stay
    ...
    def push_status(self, external_id: str, status: str, evidence: str) -> bool:
        body = f"**Kalph**: task status → `{status}`\n\n{evidence}".strip()
        return self._comment(external_id, body)

    def push_pr_link(self, external_id: str, url: str) -> bool:
        return self._comment(external_id, f"**Kalph** opened a PR: {url}")
